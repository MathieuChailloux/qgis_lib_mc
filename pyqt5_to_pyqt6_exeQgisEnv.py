...
DIRECTORY = r"C:/Users/mchailloux/Projets/QGIS_Plugins/LightPollutionToolbox"
# DIRECTORY = "/home/user/dev/MonPlugin"

QGIS3_INCOMPATIBLE_CHANGES = True
DRY_RUN = False

LOGFILE = None
# LOGFILE = r"C:\temp\qgis4_migration.log"
...
def run_in_qgis():
    ...
    logging.info("")
    logging.info("===================================")
    logging.info(" QGIS 3 -> QGIS 4 migration tool")
    logging.info("===================================")
    logging.info(f"Directory : {DIRECTORY}")
    logging.info(f"Dry run   : {DRY_RUN}")
    logging.info("")

    # Construction des tables d'énumérations Qt/QGIS
    ...
    logging.info(f"{len(py_files)} fichier(s) détecté(s)")
    logging.info("")
    ...
    for filename in py_files:
        ...
        try:
            ret |= fix_file(
                filename,
                qgis3_compat=not QGIS3_INCOMPATIBLE_CHANGES,
                dry_run=DRY_RUN,
            )

        except Exception as e:
            logging.exception(
                f"Erreur lors du traitement de {filename}: {e}"
            )
...
# Lancer directement depuis l'éditeur Python QGIS
run_in_qgis()